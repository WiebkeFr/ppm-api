...
def generate_pm4py_log(filename=None, verbose=False):
	...
	if filename.split(".")[-1]=="xes":
		...
		log_csv = pd.read_csv(filename, sep=i_d)
		...
		if log_csv.dtypes["case"] != "str":
			log_csv["case"] = log_csv["case"].apply(str)
		if log_csv.dtypes["concept:name"] != "str":
			log_csv["concept:name"] = log_csv["concept:name"].apply(str)
...
def generate_log(pm4py_log,verbose=False):
	log = []
	for trace_index, trace in enumerate(pm4py_log):
		for index, event in enumerate(trace):
			trace_index = trace.attributes['concept:name'] if 'concept:name' in trace.attributes else f"trace_{trace_index}"
			time_stamp = event['time:timestamp'] if 'time:timestamp' in event else f"2014-02-{index}T00:00:00+00:00"
			log.append(Event(trace.attributes['concept:name'], event['concept:name'], time_stamp))
